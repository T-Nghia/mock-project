...
class FakeRetriever:
    ...
    def retrieve(self, *, document_id, user_id=None, question, top_k=5):
        self.calls.append((document_id, user_id, question, top_k))
        return self.chunks
...
class ChatServiceTestCase(unittest.TestCase):
    ...
    def test_create_session_requires_existing_ready_document_with_embedding(self):
        ...
        global_session = service.create_session(document_id=None, current_user=self.user)
        self.assertEqual(global_session.user_id, self.user.id)
        self.assertIsNone(global_session.document_id)
        self.assertEqual(global_session.title, "Chat với Toàn bộ Tài liệu")
    ...
    def test_ask_uses_session_document_and_persists_answer_with_bounded_sources(self):
        ...
        self.assertEqual(retriever.calls, [(document.id, self.user.id, "Bien la gi?", 5)])
